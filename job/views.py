...
def admin_home(request):
	...
	stduser = StudentUser.objects.all()
	recuser = Recruiter.objects.all()
...
def apply_resume(request,jid):
	...
	if request.method == "POST":
		...
		try:
			user = StudentUser.objects.get(user=request.user)
			job = Jobs.objects.get(id=jid)
			UserApply.objects.create(user=user,job=job,resume=res)
			error = "no"
		except:
			error = "yes"
	context = {
